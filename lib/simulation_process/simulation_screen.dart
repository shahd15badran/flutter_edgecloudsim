...
class _SimulationScreenState extends State<SimulationScreen>
  ...
  Future<String> _getBatteryLevel() async {
    ...
    try {
      return await Platform.invokeMethod("Start Sim", user_id);
  ...
  Future<void> _getlogs() async {
    ...
    try {
      final String result =
          await Platform.invokeMethod('get log file', user_id);
      batteryLevel = ' $result  ';
    } on PlatformException catch (e) {
      print(e);
    }
  ...
  Future<void> _sendPlotGeneric() async {
    ...
    try {
      final String result = await Platform.invokeMethod(
          'updateGeneric', [user_id, display_plotGeneric(user_id)]);
    } on PlatformException catch (e) {
      print(e);
    }
  }
                    ...
                    children: <Widget>[
                      Padding(
                        ...
                        child: SizedBox(
                            width: 350,
                            child: Text(
                              'Your configuration files have been sent, you can click the button below to continue the process..',
                              style: TextStyle(
                                fontSize: 16,
                                color: Colors.white,
                      ...
                      Padding(
                        ...
                        child: SizedBox(width: 250, child: setUpMsgChild()),
                      ),
                      Padding(
                        padding:
                            ...
                            decoration: InputDecoration(
                              ...
                              disabledBorder: OutlineInputBorder(
                                  borderSide: BorderSide(
                                      color: Color(0xFF77A5CD), width: 0.0)),

                              // fillColor: Color(0xFFE7DADA),
                              // filled: true,
                              border: InputBorder.none,
                              hintText:
                                  "        Click on the check icon when it appears!",
                            ),
                          ),
                        ),
                      ),
              ...
              children: [
                Padding(
                  ...
                  child: SizedBox(
                    width: 350,
                    child: Text(
                      'When simulation details appears, you can download the generated log files, or further apply matlab functions on them to see clearer results! ',
                      style: TextStyle(
                        fontSize: 16,
                        color: Color(0xFF77A5CD),
                        fontWeight: FontWeight.w600,
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: new MaterialButton(
                      ...
                      onPressed: _isLogButtonDisabled
                          ? null
                          : () async {
                              ...
                              if (status.isGranted) {
                                ...
                                final path = Directory(
                                    "storage/emulated/0/Documents/$folderName");
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: MaterialButton(
                      ...
                      onPressed: _isPdfButtonDisabled
                          ? null
                          : () async {
                              _sendPlotGeneric();
  ...
  Widget setUpButton(String name) {
    //Pdfs Button
    return new Text(
      name,
    ...
    builder.element('applications', nest: () {
      for (int c = 0; c < 4; c++) {
        ...
        builder.element('application', nest: () {
          ...
          builder.element('usage_percentage',
              nest: prefs.getStringList('usage_percentage')[c]);
          builder.element('prob_cloud_selection',
              nest: prefs.getStringList('prob_cloud_selection')[c]);
          builder.element('poisson_interarrival',
              nest: prefs.getStringList('poisson_interarrival')[c]);
          builder.element('delay_sensitivity',
              nest: prefs.getStringList('delay_sensitivity')[c]);
          builder.element('active_period',
              nest: prefs.getStringList('active_period')[c]);
          builder.element('idle_period',
              nest: prefs.getStringList('idle_period')[c]);
          builder.element('data_upload',
              nest: prefs.getStringList('data_upload')[c]);
          builder.element('data_download',
              nest: prefs.getStringList('data_download')[c]);
          builder.element('task_length',
              nest: prefs.getStringList('task_length')[c]);
          builder.element('required_core',
              nest: prefs.getStringList('required_core')[c]);
          builder.element('vm_utilization_on_edge',
              nest: prefs.getStringList('vm_utilization_on_edge')[c]);
          builder.element('vm_utilization_on_cloud',
              nest: prefs.getStringList('vm_utilization_on_cloud')[c]);
          builder.element('vm_utilization_on_mobile',
              nest: prefs.getStringList('vm_utilization_on_mobile')[c]);
    ...
    builder.element('edge_devices', nest: () {
      for (int c = 0; c < 8; c++) {
        builder.element('datacenter', nest: () {
          ...
          builder.element('costPerBw',
              nest: prefs.getStringList('costPerBw')[c]);
          builder.element('costPerSec',
              nest: prefs.getStringList('costPerSec')[c]);
          builder.element('costPerMem',
              nest: prefs.getStringList('costPerMem')[c]);
          builder.element('costPerStorage',
              nest: prefs.getStringList('costPerStorage')[c]);
          ...
          builder.element('location', nest: () {
            ...
            builder.element('attractiveness',
                nest: prefs.getStringList('attractiveness')[c]);
          });
          builder.element('hosts', nest: () {
            builder.element('host', nest: () {
              ...
              builder.element('storage',
                  nest: prefs.getStringList('storage')[c]);
              builder.element('VMs', nest: () {
                builder.element('VM', nest: () {
                  builder.attribute('vmm', 'Xen');
                  builder.element('core',
                      nest: prefs.getStringList('core1')[c]);
                  builder.element('mips',
                      nest: prefs.getStringList('mips1')[c]);
                  builder.element('ram', nest: prefs.getStringList('ram1')[c]);
                  builder.element('storage',
                      nest: prefs.getStringList('storage1')[c]);
                });
                builder.element('VM', nest: () {
                  builder.attribute('vmm', 'Xen');
                  builder.element('core',
                      nest: prefs.getStringList('core2')[c]);
                  builder.element('mips',
                      nest: prefs.getStringList('mips2')[c]);
                  builder.element('ram', nest: prefs.getStringList('ram2')[c]);
                  builder.element('storage',
                      nest: prefs.getStringList('storage2')[c]);
                });
              });
            });
          });
  ...
  display_Configs() {
    ...
    List<String> data = [
      'simulation_time',
      'warm_up_period',
      'vm_load_check_interval',
      'location_check_interval',
      'file_log_enabled',
      'deep_file_log_enabled',
      'min_number_of_mobile_devices',
      'max_number_of_mobile_devices',
      'mobile_device_counter_size',
      'wan_propagation_delay',
      'lan_internal_delay',
      'wlan_bandwidth',
      'wan_bandwidth',
      'gsm_bandwidth',
      'number_of_host_on_cloud_datacenter',
      'number_of_vm_on_cloud_host',
      'core_for_cloud_vm',
      'mips_for_cloud_vm',
      'ram_for_cloud_vm',
      'storage_for_cloud_vm',
      'core_for_mobile_vm',
      'mips_for_mobile_vm',
      'ram_for_mobile_vm',
      'storage_for_mobile_vm',
  ...
  display_plotGeneric(String userKey) {
    String multiValues;
    String singleValue;
    String multiFlag;

    String orchPolicies = prefs.getString('orchestrator_policies');
    String simScenarios = prefs.getString('simulation_scenarios');
    int pNum = ','.allMatches(orchPolicies).length;
    int sNum = ','.allMatches(simScenarios).length;
    String scenarioType = "";
    String legends = "";
    String policyType = "";
    if (pNum == 0) {
      singleValue = orchPolicies; //TODO
      int sStart = 0;
      for (int i = 0; i < sNum; i++) {
        int index = simScenarios.indexOf(
            ",", simScenarios.indexOf(",") + i); //number of appearance..
        scenarioType += simScenarios.substring(sStart, index) +
            "','"; //print(simScenarios.substring(sStart,index));
        switch (simScenarios.substring(sStart, index)) {
          case 'SINGLE_TIER':
            legends += "1-tier','";
            break;
          case 'TWO_TIER':
            legends += "2-tier','";
            break;
          case 'MOBILE_PROCESSING_SCENARIO':
            legends += "mob proc','";
            break;
          case 'TWO_TIER_WITH_EO':
            legends += "2-tier with EO','";
            break;
        }
        sStart = index + 1;
      }
      scenarioType += simScenarios.substring(sStart, simScenarios.length);
      switch (simScenarios.substring(sStart, simScenarios.length)) {
        case 'SINGLE_TIER':
          legends += "1-tier";
          break;
        case 'TWO_TIER':
      ...
      multiValues = scenarioType;
      multiFlag = 'S';
      //print(scenarioType);
    }
    else{ //muli policy
      singleValue = simScenarios;
      int pStart = 0;
      int prevIndex = 0;
      for (int i = 0; i < pNum; i++) {
        int index = orchPolicies.indexOf(
            ",",prevIndex+1 ); //number of appearance.. //orchPolicies.indexOf(",") + i
        prevIndex = index;
        print(orchPolicies);
        print(pStart.toString()+","+index.toString());
        //print(orchPolicies.substring(pStart, index));
        policyType += orchPolicies.substring(pStart, index) +
            "','";

        switch (orchPolicies.substring(pStart, index)) {
          case 'NEXT_FIT':
            legends += "next-fit','";
            break;
          case 'ONLY_EDGE':
      ...
      policyType += orchPolicies.substring(pStart, orchPolicies.length);
      switch (orchPolicies.substring(pStart, orchPolicies.length)) {
        case 'NEXT_FIT':
          legends += "next-fit";
          break;
        case 'ONLY_EDGE':
    ...
    all +=
        'folderPath = "C:/Users/hp/eclipse-workspace/EdgeCloudSim-master/EdgeCloudSim-master/sim_results/' +
            userKey +
            '";\n';
    all += 'numOfSimulations = 1;\n';
    all += 'startOfMobileDeviceLoop =' +
        prefs.getString('min_number_of_mobile_devices') +
        ';\n';
    all += 'stepOfMobileDeviceLoop =' +
        prefs.getString('mobile_device_counter_size') +
        ';\n';
    all += 'endOfMobileDeviceLoop =' +
        prefs.getString('max_number_of_mobile_devices') +
        ';\n';
    ...
    if(multiFlag == 'S')
      all +=
        "filePath = strcat(folderPath,'/ite',int2str(s),'/SIMRESULT_',char(scenarioType(i)),'_','"+singleValue+"','_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');\n";
    else
      all +=
          "filePath = strcat(folderPath,'/ite',int2str(s),'/SIMRESULT_" + singleValue + "_',char(scenarioType(i)),'_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');\n";
    ...
    all +=
        "C = [0.55 0 0;0 0.15 0.6;0 0.23 0;0.6 0 0.6;0.08 0.08 0.08;0 0.8 0.8;0.8 0.4 0;0.8 0.8 0];\n";
    all += "if(1 == 1)\n";
